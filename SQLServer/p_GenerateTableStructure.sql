/******************************************************************************
  ...
  *Copyright(C)
  *FileName:     p_GenerateTableStructure
  *Author:
  *Version:      1.00
  *Date:         2019-05-20
  *Description:  输入表名输出表创建脚本
  *Others:
  *Function List:
  *History:
...
ALTER PROCEDURE [dbo].[p_GenerateTableStructure]
  @table_name_old sysname, --源表名
  @table_name_new sysname, --新表名
  @keep_collation BIT           = 1, --是否保留排序规则
  @keep_identity  BIT           = 1, --是否保留自增设置
  @keep_default   BIT           = 1, --是否保留默认值
  @t_sqlstr       NVARCHAR(MAX) = N'' OUTPUT
AS
BEGIN
  SET NOCOUNT ON;

  DECLARE @table_name sysname = 'dbo.' + @table_name_old; --表名，包含dbo
  ...
  SELECT @t_sqlstr = N'CREATE TABLE ' + REPLACE(@object_name, @table_name_old, @table_name_new) + CHAR(13) + N'('
                     + CHAR(13)
                     + STUFF(
    ( SELECT    CHAR(9) + ', [' + c.name + '] '
                            ...
                            END + CASE
                                  ...
                                  END + CASE
                                          WHEN dc.[definition] IS NOT NULL
                                           AND @keep_default = 1 THEN ' DEFAULT' + dc.[definition]
                                        ELSE ''
                                        END
                    + CASE
                        WHEN ic.is_identity = 1
                         AND @keep_identity = 1 THEN
      ...
      FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)'), 1, 2, CHAR(9) + ' ')
                     + ISNULL(
                       ( SELECT CHAR(9) + ', CONSTRAINT [PK_' + @table_name_new
                                + ( SELECT '_' + c.name + ''
                                    FROM   sys.index_columns ic WITH( NOWAIT )
      ...
      FOR XML PATH(N''), TYPE              ).value('.', 'NVARCHAR(MAX)'), 1, 2, '')) + ')' + CHAR(13)
                     ...
                     + ISNULL(
                         (( SELECT CHAR(13) + 'CREATE' + CASE
                                                         ...
                                                         ELSE ''
                                                         END + ' NONCLUSTERED INDEX [Index_' + @table_name_new
                                   + ( SELECT '_' + c.name
                                       FROM   index_column c
                                       WHERE  c.is_included_column = 0
                                         AND  c.index_id = i.index_id
                                       FOR XML PATH('')) + '] ON [dbo].[' + @table_name_new + '] ('
                                   + STUFF(( SELECT ', [' + c.name + ']' + CASE
